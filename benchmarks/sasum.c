...
int main(void) { 
    ...
    for (uint32_t i = 0; i < n_lengths; i++) {
        ...
        BENCH(neon_time, n_iter, n_warm, sasum_neon(x_ptr, length));
        BENCH(sve_time, n_iter, n_warm, sasum_sve(x_ptr, length));
        BENCH(sme_time, n_iter, n_warm, sasum_sme8xVGx4(x_ptr, length));
        BENCH(main_time, n_iter, n_warm, sasum(x_ptr, length));

        display_bench("sasum", neon_time, sve_time, sme_time, main_time, length, i == 0 ? 1 : 0); 
    }

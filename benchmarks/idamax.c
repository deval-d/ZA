...
int main(void) { 
    ...
    for (uint32_t i = 0; i < n_lengths; i++) {
        ...
        BENCH(neon_time, n_iter, n_warm, idamax_neon(x_ptr, length));
        BENCH(sve_time, n_iter, n_warm, idamax_sve(x_ptr, length));
        BENCH(main_time, n_iter, n_warm, idamax(x_ptr, length));

        display_bench("idamax", neon_time, sve_time, 0.0, main_time, length, i == 0 ? 1 : 0); 
    }

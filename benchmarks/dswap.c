...
int main(void) { 
    ...
    double *x_neon = malloc(max_length * sizeof(*x_neon));
    double *x_sve = malloc(max_length * sizeof(*x_sve));
    double *x_sme = malloc(max_length * sizeof(*x_sme));
    double *x_main = malloc(max_length * sizeof(*x_main));
    double *y_neon = malloc(max_length * sizeof(*y_neon));
    double *y_sve = malloc(max_length * sizeof(*y_sve));
    double *y_sme = malloc(max_length * sizeof(*y_sme));
    double *y_main = malloc(max_length * sizeof(*y_main));
    ...
    for (uint32_t i = 0; i < n_lengths; i++) {
        ...
        BENCH(neon_time, n_iter, n_warm, dswap_neon(x_neon, y_neon, length));
        BENCH(sve_time, n_iter, n_warm, dswap_sve(x_sve, y_sve, length));
        BENCH(sme_time, n_iter, n_warm, dswap_sme(x_sme, y_sme, length));
        BENCH(main_time, n_iter, n_warm, dswap(x_main, y_main, length));

        display_bench("dswap", neon_time, sve_time, sme_time, main_time, length, i == 0 ? 1 : 0); 
    }

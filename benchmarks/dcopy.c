...
int main(void) { 
    ...
    for (uint32_t i = 0; i < n_lengths; i++) {
        ...
        BENCH(neon_time, n_iter, n_warm, dcopy_neon(x_ptr, y_neon, length));
        BENCH(sve_time, n_iter, n_warm, dcopy_sve(x_ptr, y_sve, length));
        BENCH(sme_time, n_iter, n_warm, dcopy_sme(x_ptr, y_sme, length));
        BENCH(main_time, n_iter, n_warm, dcopy(x_ptr, y_main, length));

        display_bench("dcopy", neon_time, sve_time, sme_time, main_time, length, i == 0 ? 1 : 0); 
    }

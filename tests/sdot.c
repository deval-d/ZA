...
float sdot_ref(float *x_ptr, float *y_ptr, uint32_t length) { 
    float sum = 0;
...
int main(void) { 
    ...
    float *y_ptr = random_vec_float(max_length);
    ...
    for (uint32_t j = 0; j < n_lengths; j++) { 
        ...
        sum_ref = sdot_ref(x_ptr, y_ptr, length);
        neon = sdot_neon(x_ptr, y_ptr, length);
        sve = sdot_sve(x_ptr, y_ptr, length);
        sme4xVGx4 = sdot_sme4xVGx4(x_ptr, y_ptr, length);
        sme8xVGx4 = sdot_sme8xVGx4(x_ptr, y_ptr, length);
